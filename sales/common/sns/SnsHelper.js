...
const SALES_TOPIC_ARN = `${process.env.AWS_SNS_TOPIC_ARN_PREFIX}:demo-${process.env.LOCALSTACK_ENDPOINT ? 'local' : process.env.NODE_ENV}-sales`
 ...
 * @param {?String} topicArn - Topic ARN to publish on. Defaults to the sales SALES_TOPIC_ARN.
 * @returns {?Promise<SnsResponseObject>} - This returns
 */
function publish (subject, message, topicArn = SALES_TOPIC_ARN) {
...
const SNS = {
  salesCreated: (event) => {
    return publish('Sales:created', event)
  }
